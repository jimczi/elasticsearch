        ...
        withColumn(
            ...
            (metadata, reader) -> {
                ...
                for (String probe : probes) {
                    ...
                    for (Predicate<String> shape : List.<Predicate<String>>of(v -> v.equals(probe), v -> v.startsWith(probe))) {
                        final FixedBitSet expected = new FixedBitSet(docSlots.length);
                        for (int d : expected(docSlots, shape)) {
                            expected.set(d);
                        }
                        final boolean exact = shape.test(probe + "x") == false;
                        assertDocIDRunEndContract(
                            (exact ? "term [" : "prefix [") + probe + "]",
                            () -> exact ? reader.matchTerm(term) : reader.matchPrefix(term),
                            expected,
                            docSlots.length
                        );
                    }
