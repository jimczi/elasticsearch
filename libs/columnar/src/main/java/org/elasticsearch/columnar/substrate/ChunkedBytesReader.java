...
public final class ChunkedBytesReader {
     ...
     * call on this reader. A caller that reads a run of values and hands each one out in turn saves a copy
     * per value this way; one that keeps them must copy.
     */
    public void span(long offset, int length, BytesRef dst) throws IOException {
        ...
        if (codec.isIdentity()) {
            ...
            if (verbatim.length < length) {
                verbatim = new byte[ArrayUtil.oversize(length, Byte.BYTES)];
            }
            data.seek(dataOffset + offset);
