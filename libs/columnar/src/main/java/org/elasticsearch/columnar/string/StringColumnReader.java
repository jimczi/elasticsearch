...
public abstract sealed class StringColumnReader permits PlainStringColumnReader, DictionaryStringColumnReader {
    ...
    protected static class SlotWindow {
        ...
        /** The first slot at or after {@code from} the window holds, or {@code -1}. */
        final long next(long from) throws IOException {
            final long end = blocks.numValues();
            while (from < end) {
                final long window = from >>> shift;
                load(window);
                final int at = bits.nextSetBit((int) (from & mask));
                if (at != DocIdSetIterator.NO_MORE_DOCS) {
                    return (window << shift) + at;
                }
                from = (window + 1) << shift;
            }
            return -1;
        ...
        /** The first slot at or after {@code from} the window does not hold, or the end of the blocks. */
        final long runEnd(long from) throws IOException {
            final long end = blocks.numValues();
            while (from < end) {
                final long window = from >>> shift;
                load(window);
                final int at = bits.nextClearBit((int) (from & mask));
                if (at != DocIdSetIterator.NO_MORE_DOCS) {
                    return (window << shift) + at;
                }
                from = (window + 1) << shift;
