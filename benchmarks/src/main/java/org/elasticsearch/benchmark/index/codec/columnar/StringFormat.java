...
public enum StringFormat {
    ...
    long write(Directory directory, BytesRef[] values, int chunkBytes, int valuesPerBlock) throws IOException {
        if (isColumnar()) {
            return writeColumnar(directory, values, chunkBytes);
        }
    ...
    private long writeColumnar(Directory directory, BytesRef[] values, int chunkBytes) throws IOException {
        final FieldType type = new FieldType();
        type.setDocValuesType(DocValuesType.BINARY);
        type.putAttribute("columnar.type", "STRING");
        type.freeze();
    ...
    private static final class ColumnarColumn implements Column {
        ...
        public long aggregate() throws IOException {
            ...
            if (dictionarySize == 0) {
                // No ordinals: every value has to be hashed, which is map-mode aggregation.
                final BytesRefHash groups = new BytesRefHash();
                final BytesRef scratch = new BytesRef();
                long checksum = 0;
                for (int doc : docs) {
                    checksum += StringFormat.group(groups, reader.valueAt(doc, scratch));
                }
                return checksum;
            }
