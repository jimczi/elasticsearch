...
public class SemanticInferenceMetadataFieldsRecoveryTests extends EngineTestCase {
    ...
    public void testSnapshotRecovery() throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            expectedOperations.add(toTranslogIndexOp(op, result.getSeqNo(), result.getVersion()));
            if (frequently()) {
                engine.flush();
            }
        }
        ...
        try (
            ...
            var translogAsserter = TranslogOperationAsserter.withEngineConfig(engine.config());
            for (int i = 0; i < size; i++) {
                var op = snapshot.next();
                assertThat(op.opType(), equalTo(Translog.Operation.Type.INDEX));
                Translog.Index indexOp = (Translog.Index) op;
                translogAsserter.assertSameIndexOperation(indexOp, expectedOperations.get(i));
            }
            assertNull(snapshot.next());
        }
    }

    private Translog.Index toTranslogIndexOp(Engine.Index op, long seqNo, long version) {
        return new Translog.Index(op.id(), seqNo, op.primaryTerm(), version, op.source(), op.routing(), op.getAutoGeneratedIdTimestamp());
    }
